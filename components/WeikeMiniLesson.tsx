...
export default function WeikeMiniLesson({ level, weekNumber }: WeikeMiniLessonProps) {
  ...
  const lessons: Record<string, LessonContent> = {
    ...
    "SMATH-63": {
      examples: [
        "2x + 6x + 4y + 12y = 8x + 16y = 8(x + 2y)",
        "3a + 9a + 5b + 15b = 12a + 20b = 4(3a + 5b)"
      ],
      gloss: "AEIS 中学数学第 63 周 / SMATH Week 63. 本周例题：Sec 2 N5.13 factorisation of linear expressions ax+bx+kay+kby（一次式因式分解 / 分组提取）。官方 5.13 wording（逐字引用）：'factorisation of linear expressions ax+bx+kay+kby'。本周是 N5.13 only. Group four-term linear expressions: ax + bx + kay + kby. Method: Step 1: Combine like terms in the first pair (ax + bx). Step 2: Combine like terms in the second pair (kay + kby). Step 3: Factor the resulting two-term expression by finding the GCF. Example: 2x + 6x + 4y + 12y → Combine: 2x + 6x = 8x, 4y + 12y = 16y → Factor: 8x + 16y = 8(x + 2y). Friendly integers only. No calculator. No quadratics (no x² terms to factor). Fossil: taking out only one pair and stopping (example: stopping at 8x + 16y without factoring to 8(x + 2y)); or writing (a+b)(x+y) when the coefficient is wrong (example: 10x + 30y = 10(x + y) ✗, should be 10(x + 3y) ✓). AEIS 不允许使用计算器 (calculators are not allowed), so friendly integers only. Verbal markers: 'Factorise completely' (完全因式分解), 'What is the fully factorised form?' Key trap: Do NOT stop at the combined form (like 8x + 16y). You MUST factor out the GCF to get the fully factorised form (like 8(x + 2y)). Cast: secondary Wei, Aisha, Mr Lim at Riverside Secondary. 本周不教 5.14 factorisation of quadratic expressions ax²+bx+c. 本周不重教 5.12 identities（第 62 周已教）。本周不重教 Sec 1 N5.8 common-factor extraction as the whole week（第 56 周已教；本周是分组：ax + bx + kay + kby）。第 8–56 周已完成 Sec 1 内容。第 57 周已完成 Sec 2 N2.4 map scales。第 58 周已完成 Sec 2 N2.5 direct and inverse proportion。第 59 周已完成 Sec 2 N5.9 expansion of the product of algebraic expressions。第 60 周已完成 Sec 2 N5.10 changing the subject of a formula。第 61 周已完成 Sec 2 N5.11 finding the value of an unknown quantity in a given formula。第 62 周已完成 Sec 2 N5.12 identities: (a+b)², (a−b)², a²−b²。本周继续 Sec 2 内容（Sec 3 申请者的 preceding level）。"
    },
    "SEC-0": {
